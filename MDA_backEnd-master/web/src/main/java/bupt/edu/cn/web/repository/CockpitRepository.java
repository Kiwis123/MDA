...
public interface CockpitRepository extends JpaRepository<Cockpit, Integer>, JpaSpecificationExecutor<Cockpit> {
    @Transactional
    @Query(value = "select * from cockpit where id=?1", nativeQuery = true)
    public Cockpit findById(int CockId);

    @Transactional
    @Query(value = "select * from cockpit where user_id=?1", nativeQuery = true)
