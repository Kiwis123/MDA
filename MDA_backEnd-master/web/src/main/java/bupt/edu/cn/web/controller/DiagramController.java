...
//            chartModelOne.se
    ...
    public ReturnModel saveDiagram(String projectId, String workId, String diagramId, String diagramName,String option, String classificaion, String userId, String dataSourceId, HttpServletResponse response, HttpServletRequest request) {
        // 解决Ajax跨域请求问题
        response.setHeader("Access-Control-Allow-Origin", request.getHeader("Origin"));
        response.setHeader("Access-Control-Allow-Credentials", "true");
...
//        List<Diagram> diagramList = diagramRepository.findByUserId(userId);
        //将option由JSONString改为JSON
        JSONArray jsonArray = new JSONArray();
        for (int i=0;i<diagramList.size();i++){
            JSONObject item =new JSONObject();
            System.out.println(diagramList);
            System.out.println(diagramList.get(i).getChart());
            item.put("option", 1);
