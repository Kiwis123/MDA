...
export default {
  ...
  data() {
    return {
      namespaces: [],
      // namespaces: [{name:"电商用户行为分析", descr: "追踪用户转化路径，分析功能活跃度，提升产品体验", status: 1}, {name:"卫健委优生优育专题", descr: "分析新生儿出生缺陷、妊娠结果、父母体检及回访情况", status: 2}],
      loading: true,
      modal_state: {
        create: false,
        update: false,
  ...
  methods: {
    enterIde(id) {
      this.$router.push({ 'name': 'mda', query: { projectId: id } })
    },
    addNameSpace() {
      ...
      params.append("description", this.new_namespace.descr);
      params.append("user", "tc");
      this.modal_state.modalLoading = true;
      this.axios.post("http://localhost:6993/addProject", params).then(res => {
    ...
    getNameSpaces() {
      this.loading = true;
      this.namespaces = [];
      this.axios.get("http://localhost:6993/getProjectList").then(res => {
        if (res.status == 200) {
          // 格式转换
          let projectList = res.data.datum;
    ...
    updateNameSpace() {
      ...
      params.append("description", descr);
      this.modal_state.modalLoading = true;
      this.axios.put("http://localhost:6993/updateProject", params).then(res => {
        this.modal_state.modalLoading = false;
        if (res.status == 200) {
